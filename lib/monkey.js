...
// --- Record-breaking streak call-outs ("on this day" moments) ---

const LEAGUE_RECORD_LINES = [
  " HOLD THE JUNGLE. {streak} wins in a row is a new LEAGUE RECORD. No monkey has ever run this hot. Frame this moment, {ref}.",
  " That's {streak} straight wins - nobody in Rock My Billys history has ever strung together a run like this. History has been made, {ref}.",
  " New league record: {streak} in a row. Even I'm a little impressed, and I don't respect any of you.",
];

const PERSONAL_RECORD_LINES = [
  " That's a new PERSONAL best for you, {ref} - {streak} wins in a row. Don't let it go to your head, it always does.",
  " {streak} in a row is your best run yet. The old you would be proud. The old you was still trash though.",
  " New personal record, {streak} straight. Enjoy it, {ref}, it's the closest thing to greatness you'll get.",
];

/**
 * Returns an extra sentence to bolt onto the winner's message when their
 * fresh win streak breaks either their own personal best or the all-time
 * league record, or null if nothing noteworthy happened.
 */
export function getStreakRecordLine({ streak, isLeagueRecord, isPersonalRecord, playerName }) {
  if (streak < 2) return null;
  const vars = { streak, ref: ref(), winner: playerName };
  if (isLeagueRecord) return fillTemplate(pick(LEAGUE_RECORD_LINES), vars);
  if (isPersonalRecord) return fillTemplate(pick(PERSONAL_RECORD_LINES), vars);
  return null;
}
